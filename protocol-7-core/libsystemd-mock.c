/*
 * libsystemd-mock.c – Protocol 7 Sovereign ABI Provider (v4.3)
 *
 * ABI-compatible drop-in replacement for libsystemd.so.0
 * Targeted at: Electron, Chromium, Firefox, xdg-desktop-portal, PipeWire,
 * Flatpak, Wayland compositors, and modern 2025-2026 desktop stacks.
 *
 * Hardened + Improved: unified init, architecturally-safe fake pointers,
 * RNG seeding, enhanced error handling (correct return values), smart
 * D-Bus responses, message type accessors, process/wait loops, match
 * rules, verbose logging mode.
 *
 * Compile:
...
// ─── Fake pointers (x86_64 non-canonical hole → guaranteed #GP on deref) ───
...
static const struct {
    ...
    .n_type   = 0x5057,
    .name     = "LainOS\0\0",
    .desc     = "P7-ABI-2026.05|v4.3|sd_session|sd_seat|sd_uid|sd_pidfd|sd_peer|"
                "sd_bus_creds|sd_id128|sd_path|sd_is|sd_watchdog|sd_bus_error|sd_notifyf|"
                "sd_bus_process|sd_bus_wait|sd_bus_add_match|sd_bus_message_type"
};
#endif

const char protocol7_libsystemd_abi_version[] __attribute__((used)) =
    "P7-ABI-2026.05|v4.3|sd_session|sd_seat|sd_uid|sd_pidfd|sd_peer|sd_bus_creds|sd_id128|"
    "sd_path|sd_is|sd_watchdog|sd_bus_error|sd_notifyf|sd_bus_process|sd_bus_wait|"
    "sd_bus_add_match|sd_bus_message_type";
...
static void p7_log(const char *fmt, ...) {
    ...
    va_list ap;
    va_start(ap, fmt);
    vfprintf(stderr, "[P7-MOCK] ", ap);
    vfprintf(stderr, fmt, ap);
    fprintf(stderr, "\n");
    va_end(ap);
}

/* ─── Unified constructor: RNG seed + version guard ─── */
static void __attribute__((constructor)) p7_init(void) {
    ...
    const char *too_new[] = {
        "systemd 256", "systemd 257", "systemd 258", "systemd 260",
        "sd-bus v256", "sd-bus v257", NULL
    };

    for (int i = 0; too_new[i]; i++) {
        if (strstr(buf, too_new[i])) {
            openlog("p7-mock", LOG_PID, LOG_DAEMON);
            syslog(LOG_CRIT, "Incompatible systemd version detected: %s", too_new[i]);
            closelog();
            fprintf(stderr, "[P7] ABORT: libsystemd-mock incompatible with real systemd\n");
            _exit(127);
        }
    }
}
...
int sd_id128_get_machine(sd_id128_t *ret) {
    ...
    if (f) {
        ...
        if (fgets(buf, sizeof(buf), f)) {
            ...
            for (int i = 0; i < 16 && buf[i*2] && buf[i*2+1]; i++) {
                int hi = strchr(hex, buf[i*2]) ? (int)(strchr(hex, buf[i*2]) - hex) : 0;
                int lo = strchr(hex, buf[i*2+1]) ? (int)(strchr(hex, buf[i*2+1]) - hex) : 0;
                (*ret)[i] = (hi << 4) | lo;
            }
        }
        fclose(f);
...
int sd_id128_randomize(sd_id128_t *ret) {
    ...
    memset(ret, 0, 16);
    memcpy(ret, &ts.tv_nsec, sizeof(long));
    return 0;
}
